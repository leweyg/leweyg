...
function evxToolsString(obj) {
	if (obj == undefined) return "";
	if (obj == null) return "";
	return "" + obj;
}

function evxToolsStringCompare(a,b) {
	if (a < b) return -1;
	if (a > b) return 1;
	return 0;
}
...
function evxToolsClampM(v,mn,mx) {
	if ((v >= mn) && (v <= mx)) {
		return v;
	} else if (v < mn) {
		return mn;
...
function evxToolsVectorAdd(a, b, into) {
	into.x = (a.x + b.x);
	into.y = (a.y + b.y);
	into.z = (a.z + b.z);
	return into;
...
function evxToolsUrlWithCacheBypass(url) {
	var prefix = ((url.includes("?")) ? ("&") : ("?"));
	url += prefix + "at=" + ((new Date()).getTime());
	return url;
}
...
function evxToolsJsonObjectToString(obj, filter=undefined) {
	return JSON.stringify(obj,undefined);
	// example: evxToolsJsonToString(shape,function(key,val){
	//	if (key != 'evxTag') return val;
	//	return null;
	//});
}
...
function evxObjThreeCreateTexture(path, onErrorCallback, onSuccessCallback) {
	if (path in __evxCachedTextures) {
		return __evxCachedTextures[path];
	}
	var ans = new THREE.TextureLoader().load( path,
		function(loadedImage) { 
			if (evxToolsNotNull(onSuccessCallback)) {
				onSuccessCallback();
			}
			evxRequestUpdate(); 
		},
...
function evxObjectThreeCreateBackgroundSphere(path,imageScale) {
	var geometry = new THREE.SphereGeometry( 20, 24, 12 );
	var texture = evxObjThreeCreateTexture( path );

	var scale360 = (imageScale || 1.45);
    ...
    for (var isi in shape.IndexSets) {
        ...
        if (ss.IndexType == 'Points') {
			...
			if (false) {
				...
				var clr = undefined;
				if (evxToolsHasOption(ss,'LineColor')) {
					clr = new THREE.Color( ss.Options.Strings.LineColor );
				}
				mesh = evxShapeIndexedLineCreateFancyLines(shape, ss, null, isAnts, clr);
			}
...
function evxToolsUrlGetArg(name) {
...
function evxShapeIndexedLineCreateFancyLines(shape,ns,group=null,curvyCorners=false, defColor=null) {
	var positions = [];
	var nextPoses = [];
	var uvs = [];
	var evxcolors = null;
	...
	if (evxToolsIsNull(defColor)) {
		defColor = new THREE.Color( 0xA9F3B3 );
	}
...
function evxShapeFollowCharacterFindPathPos(shape,ss,z,into,intoDir) {
	var positions = shape.PackedUnitVertices;
    var vertexWidth = shape.Scope.Packing.VertexWidth;
	...
	var zIndex = 2;
	var isYAxis = false;
	if (evxToolsHasOption(ss, "FollowAxis")) {
		if (ss.Options.Strings.FollowAxis == "y") {
			isYAxis = true;
			zIndex = 1;
			yIndex = 2;
		}
	...
	for (var ii=0; ii<indices.length; ii+=lc) {
		...
		if ((isMatch) || (isEdge)) {
			...
			into.z = z;
			into.x = evxToolsLerp( positions[ (indices[ii+0] * vertexWidth) + xIndex ], positions[ (indices[ii+si] * vertexWidth) + xIndex ], zf );
			into.y = evxToolsLerp( positions[ (indices[ii+0] * vertexWidth) + yIndex ], positions[ (indices[ii+si] * vertexWidth) + yIndex ], zf );
			if (evxToolsNotNull(intoDir)) {
				intoDir.x = positions[ (indices[ii+0] * vertexWidth) + xIndex ] - positions[ (indices[ii+si] * vertexWidth) + xIndex ];
				intoDir.y = positions[ (indices[ii+0] * vertexWidth) + yIndex ] - positions[ (indices[ii+si] * vertexWidth) + yIndex ];
				intoDir.z = 0;
			}
			if (isYAxis) {
				var t = into.z;
...
var evx_HACKY_cachedMaterial = undefined;
function evxShapeFollowCharacterAdd(group,shape,ss) {
	...
	if (evxToolsNotNull(ss.Options) && ('is_plane' in ss.Options.Strings)) {
		isPerson = false;
		isAirplane = true;
		worldScale = 0.05;
	}
	...
	if (true) {
		...
		if (isPerson) {
			evx_3dPack_LoadModelGeneric("models/poly_people.json", function (packOfPeople) {
				...
				var mat = evx_3dPack_SetMaterialColorFromHex(person, "#eabd07");
				evx_HACKY_cachedMaterial = mat;
				sphere.add(person);
				evxRequestUpdate();
				}
			});
		} else if (isAirplane) {
			evx_3dPack_LoadModelObj("models/poly_airplane.obj", "", function(planeModel) {
				if (planeModel) {
					var person = evx_3dPack_CreateItemInstance( planeModel );
					//evx_3dPack_SetScale(person, 0.41);
					if (evx_HACKY_cachedMaterial) {
						person.traverse(function(c) { 
							if (evxToolsNotNull(c.material)) {
								c.material = evx_HACKY_cachedMaterial;
							}
						});
					}
	...
	var followDir = new THREE.Vector3(0,1,0);
	var scaleCount = 0;
	var curFwd = true;
	var isWorldScale = false;
	var worldScaleScales = [];

	sphere.evxOnScrollChange = function(scrollInput) {
		var z = 0;
		if (scrollInput){
			var zScope = scrollInput.Vector[2].Scope;
			z = evxToolsLerp (zScope.From, zScope.To, 0.5);
		}
		if (sphere.evxScrollCustomValue) {
			z = sphere.evxScrollCustomValue();

			isWorldScale = true;
			
		}
		// do something with this
		var fwd = ( evxShapeFollowCharacterFindPathPos(shape, ss, z, followPos, followDir ) );
		if (!evxToolsHasOption(ss, "FollowAxis")) {
			followPos.z = z;
		} else {
			followPos.y = z;

			var ang = Math.atan2(-followDir.z, followDir.x);
			sphere.rotation.y = ang - (Math.PI/2.0);
		}
		...
		if (isAirplane) {
			if (followDir.lengthSq() > 0) {
				var ang = Math.atan2(followDir.y, followDir.x);
				sphere.rotation.y = ang + Math.PI;
			}
		}
		if (isWorldScale) {
			if (worldScaleScales.length < 3) {
				for (var si=0; si<3; si++) {
					worldScaleScales.push( new THREE.Vector3() );
				}
			}
			var worigin = worldScaleScales[0];
			var wscl = worldScaleScales[1];
			var wres = worldScaleScales[2];

			var sclRef = sphere.parent;

			var goalScl = 0.25;
			worigin.set(0,0,0);
			sclRef.worldToLocal(worigin);

			wscl.set(goalScl,0,0);
			sclRef.worldToLocal(wscl);
			wres.x = evxToolsVectorDistance(worigin, wscl);

			wscl.set(0,goalScl,0);
			sclRef.worldToLocal(wscl);
			wres.y = evxToolsVectorDistance(worigin, wscl);

			wscl.set(0, 0, goalScl);
			sclRef.worldToLocal(wscl);
			wres.z = -evxToolsVectorDistance(worigin, wscl);

			sphere.scale.copy(wres);
		}
...
function evxTagLabelsUpdateLocation(all, label, objThree, pos ) {
	...
	if (evxToolsNotNull(objThree.evxTagEl) 
			&& evxToolsNotNull(objThree.evxTagEl.clipMatrix)
    ...
    var isLeft = (scPos.x >= 0.0) || (!label.isAutoAlign);
    if (isLeft) {
...
function evx_3dPack_LoadModelObj(dirPath, modelPath, sceneCallback) {
	if ((dirPath + modelPath) == "") {
		sceneCallback(undefined);
		return;
	}
...
function evxShaderPixelUnlitMonoTexture(prefixes="",postVal = "") {
	var decls = ""
    + "uniform sampler2D map;"
    //+ "uniform vec4 uvTransform;"
	+ "varying vec2 vUv;"
...
function evxShaderMaterialCreateForUnlitMonoTexture(defColor,txLoader,uvTransform,postfix) {
	var material = new THREE.ShaderMaterial( {
		uniforms: {
			//amplitude: { value: 1.0 },
			color:     { value: ( defColor ) },
		...
		vertexShader:   evxShaderBasicVertexWithUVs(),
		fragmentShader: evxShaderPixelUnlitMonoTexture("",postfix),
		blending:       THREE.NormalBlending,
		depthTest:      true,
		transparent:    true,
		side: 			THREE.DoubleSide,
